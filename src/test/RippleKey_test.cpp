...
private:
    ...
    testRandom(ripple::KeyType const kt)
    {
        ...
        auto const pubkey1 =
            toBase58(TokenType::AccountPublic, key1.publicKey());
        auto const pubkey2 =
            toBase58(TokenType::AccountPublic, key2.publicKey());
    ...
    testSeed(ripple::KeyType const kt)
    {
        ...
        auto const otherkeys = {
            ...
            RippleKey::make_RippleKey(kt, strHex(seed)),
    ...
    testFile(ripple::KeyType const kt)
    {
        ...
        auto badFile = [&](const char* toWrite,
            ...
            catch (std::exception const& e)
            {
                BEAST_EXPECT(
                    e.what() ==
                    std::string{expectedException + badKeyFile.string()});
            }
        };
        // No file
            ...
            R"(Invalid 'key_type' field "sha1" found in key file: )");
        {
            ...
            {
                std::ofstream o(badPath.string(), std::ios_base::trunc);
            }
            try
            {
            ...
            {
                BEAST_EXPECT(
                    e.what() == "Cannot create directory: " + badPath.string());
            }
            remove(badPath);
            ...
            {
                BEAST_EXPECT(
                    e.what() == "Cannot open key file: " + keyFile.string());
            }
            remove_all(badPath);
        }
        ...
        BEAST_EXPECT(
            jKeys[jss::account_id] == toBase58(calcAccountID(key.publicKey())));
        ...
        BEAST_EXPECT(jKeys[jss::master_seed_hex] == strHex(seed));
        BEAST_EXPECT(
            jKeys[jss::public_key] ==
            toBase58(TokenType::AccountPublic, key.publicKey()));
        BEAST_EXPECT(jKeys[jss::public_key_hex] == strHex(key.publicKey()));
        BEAST_EXPECT(
            jKeys["secret_key"] ==
            toBase58(TokenType::AccountSecret, secretKey));
        BEAST_EXPECT(jKeys["secret_key_hex"] == strHex(secretKey));
    }
    ...
    testSign(ripple::KeyType const kt)
    {
        ...
        auto check = tx->checkSign(STTx::RequireFullyCanonicalSig::yes, rules);
        BEAST_EXPECT(check);
        {
            auto const key =
                RippleKey::make_RippleKey(kt, std::string("alice"));
            auto const expectedSignature = kt == KeyType::secp256k1
                ? tx->getFieldVL(sfTxnSignature)
                : *strUnHex(
                      "0751E8D38C26E8B6C953766A8A58570CA0CB93E57B86047F1FEF8DA3"
                      "D7"
                      "9DFB97E78F4E59365C88EEE0E94EF7C1A2155A828B239AC00F3E9580"
                      "2D"
                      "851ABB113F06");
            auto const expectedSigningKey = kt == KeyType::secp256k1
                ? tx->getFieldVL(sfSigningPubKey)
                : *strUnHex(
                      "ED4A9D72F2557B714713DC8BA7C6F9576BCC06117A52F6C32"
                      "F1E26FEEF9819EC8E");

            // Remove the signature
            tx->makeFieldAbsent(sfTxnSignature);
            tx->setAccountID(sfAccount, calcAccountID(key.publicKey()));
            check = tx->checkSign(STTx::RequireFullyCanonicalSig::yes, rules);
            BEAST_EXPECT(!check);
...

        {
            ...
            if (BEAST_EXPECT(tx->isFieldPresent(sfSigners)))
            {
                ...
                BEAST_EXPECT(
                    to_string(signer.getAccountID(sfAccount)) ==
                    expectedAccount);
                BEAST_EXPECT(
                    strHex(signer.getFieldVL(sfSigningPubKey)) ==
                    expectedPubKey);
                BEAST_EXPECT(
                    strHex(signer.getFieldVL(sfTxnSignature)) ==
                    expectedSignature);
                BEAST_EXPECT(to_string(tx->getTransactionID()) == expectedHash);
            }
