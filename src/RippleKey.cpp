    ...
    std::optional<std::string> const& rawseed)
{
    if (keyType && rawseed)
    {
        ...
        if (!seed)
            throw std::runtime_error("Unable to parse seed: " + *rawseed);

        return RippleKey{*keyType, *seed};
    }
    if (rawseed)
        return RippleKey::make_RippleKey(RippleKey::defaultKeyType(), *rawseed);
    if (keyType)
...
RippleKey
RippleKey::make_RippleKey(boost::filesystem::path const& keyFile)
{
    ...
    for (auto field : requiredFields)
    {
        if (!jKeys.isMember(field))
        {
            throw std::runtime_error(
                ...
                "' is missing from key file: " + keyFile.string());
        }
    }
    ...
    {
        throw std::runtime_error(
            "Invalid 'key_type' field \"" + jKeys[jss::key_type].asString() +
            "\" found in key file: " + keyFile.string());
    }
    ...
    ripple::STObject& obj) const
{
    ...
    if (hashPrefix)
    {
        auto const type = publicKeyType(keys_.publicKey.slice());
        if (!type)
        {
            throw std::runtime_error("Invalid public key");
        }
        ripple::sign(obj, *hashPrefix, *type, keys_.secretKey);
    }
    else
    {
        Serializer s;
        ...
        STObject obj2(s.slice(), sfGeneric);
        std::cout << obj2.getJson(JsonOptions::none).toStyledString()
                  << std::endl;
        std::cout << strHex(s.peekData()) << std::endl;

        auto const result =
            ripple::sign(keys_.publicKey, keys_.secretKey, s.slice());
        set(obj, sfMessageKey, result);
    }
}
